    ...
    private var welcomeSection: some View {
	    ...
	    private var featureCards: some View {
	        VStack(spacing: Theme.Spacing.medium) {
                ...
                // 出席紀錄
                NavigationLink(destination: AttendanceRecordView()) {
                    HStack(spacing: Theme.Spacing.medium) {
...
private enum AttendanceStatus: String, Codable {
    case present
    case late
    case absent
    case excused
...
private struct AttendanceRecord: Codable {
    let dateKey: String
    let periodId: String
...
private struct AttendanceSession: Identifiable {
    ...
    var id: String {
        encodedAttendanceKey([String(dayIndex), periodId, course.name, course.teacher ?? "", course.classroom ?? ""])
    }
}

private struct CourseAttendanceStat: Identifiable {
    let courseKey: String
    let courseName: String
    let teacher: String?
    let classroom: String?
    let presentEquivalent: Double
    let totalForRate: Int

    var id: String { courseKey }
    var rate: Double { totalForRate > 0 ? presentEquivalent / Double(totalForRate) : 0 }
}

@MainActor
private final class AttendanceRecordViewModel: ObservableObject {
    @Published var selectedDate = Date()
    @Published private(set) var schedule: ClassSchedule?
    @Published private(set) var records: [String: AttendanceRecord] = [:]
    @Published private(set) var courseStats: [CourseAttendanceStat] = []

    private let scheduleCacheKey = "classSchedule.v2.cachedData"
    private let attendanceCacheKey = "attendance.records.v1"
    ...
    func loadScheduleFromCache() {
        guard let data = UserDefaults.standard.data(forKey: scheduleCacheKey),
              let cached = try? JSONDecoder().decode(ClassSchedule.self, from: data) else {
            schedule = nil
            return
        }
        schedule = cached
    ...
    private func loadRecords() {
        records = loadRecordsFromDisk()
        recomputeCourseStats()
    }

    private func loadRecordsFromDisk() -> [String: AttendanceRecord] {
        guard let data = UserDefaults.standard.data(forKey: attendanceCacheKey),
              let decoded = try? JSONDecoder().decode([AttendanceRecord].self, from: data) else {
            return [:]
        }
        return Dictionary(uniqueKeysWithValues: decoded.map { ($0.sessionKey, $0) })
    }

    private func scheduleSave(snapshot: [String: AttendanceRecord]) {
        saveWorkItem?.cancel()
        let records = snapshot
        let key = attendanceCacheKey
    ...
    private func recomputeCourseStats() {
        let grouped = Dictionary(grouping: records.values, by: \.courseKey)
        courseStats = grouped.map { key, values in
            let first = values[0]
            let equivalent = values.compactMap { $0.status.score }.reduce(0, +)
            let totalForRate = values.filter { $0.status.score != nil }.count
            return CourseAttendanceStat(
                courseKey: key,
                courseName: first.courseName,
                teacher: first.teacher,
                classroom: first.classroom,
                presentEquivalent: equivalent,
                totalForRate: totalForRate
            )
        }
        .sorted {
            if $0.rate == $1.rate {
    ...
    private func scheduleColumnIndex(for date: Date) -> Int? {
        guard let schedule else { return nil }
        let index = dayIndex(for: date)
        guard index < Self.weekdayNames.count else { return nil }
        let weekdayName = Self.weekdayNames[index]
        return schedule.dayHeaders.firstIndex(of: weekdayName)
    }

    var sessionsForSelectedDate: [AttendanceSession] {
        guard let schedule, let column = scheduleColumnIndex(for: selectedDate) else {
            return []
        }
        return schedule.periods.compactMap { period in
            guard let course = period.course(for: column) else { return nil }
    ...
    private func sessionKey(for session: AttendanceSession) -> String {
        AttendanceRecord(
    ...
    func status(for session: AttendanceSession) -> AttendanceStatus? {
        records[sessionKey(for: session)]?.status
    }

    func toggle(_ target: AttendanceStatus, for session: AttendanceSession) {
        var latest = loadRecordsFromDisk()
        let key = sessionKey(for: session)
        if latest[key]?.status == target {
    ...
    var totalPresentEquivalent: Double {
        records.values.compactMap { $0.status.score }.reduce(0, +)
    }

    var totalCountForRate: Int {
        records.values.filter { $0.status.score != nil }.count
    }

    var totalAttendanceRate: Double {
        guard totalCountForRate > 0 else { return 0 }
        return totalPresentEquivalent / Double(totalCountForRate)
    }

}

private struct AttendanceRecordView: View {
    @StateObject private var vm = AttendanceRecordViewModel()
    ...
    var body: some View {
        ...
        .navigationTitle("出席紀錄")
        .navigationBarTitleDisplayMode(.inline)
        .toolbar {
            ToolbarItem(placement: .navigationBarTrailing) {
                Button {
                    vm.loadScheduleFromCache()
                } label: {
    ...
    private var summaryCard: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            Text(vm.totalCountForRate == 0 ? "--" : percent(vm.totalAttendanceRate))
                .font(.system(size: 34, weight: .bold))
                .foregroundColor(.primary)

            Text("計入 \(vm.totalCountForRate) 堂，等效出席 \(String(format: "%.1f", vm.totalPresentEquivalent)) 堂")
                .font(.system(size: 13, weight: .light))
                .foregroundColor(.primary.opacity(0.6))

            Text("遲到與出席同樣計入，請假不計入出席率")
    ...
    private var todaySessionsCard: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            if vm.schedule == nil {
                Text("尚未找到課表快取，請先到「我的課表」載入一次課表")
                    .font(.system(size: 14))
                    .foregroundColor(.primary.opacity(0.55))
            } else if vm.sessionsForSelectedDate.isEmpty {
                Text("這一天沒有課")
                    .font(.system(size: 14))
                    .foregroundColor(.primary.opacity(0.55))
            } else {
                ForEach(vm.sessionsForSelectedDate) { session in
                    sessionRow(session)
                }
            }
        }
    ...
    private func sessionRow(_ session: AttendanceSession) -> some View {
        let selected = vm.status(for: session)
        return VStack(alignment: .leading, spacing: 8) {
            HStack(alignment: .top, spacing: 8) {
                VStack(alignment: .leading, spacing: 3) {
                    ...
                    Text("第\(session.periodId)節  \(session.timeRange)")
                        .font(.system(size: 12, weight: .light))
                        .foregroundColor(.primary.opacity(0.5))

                    if let details = session.course.details {
                        Text(details)
                            .font(.system(size: 12, weight: .light))
                            .foregroundColor(.primary.opacity(0.5))
                    }
            ...
            HStack(spacing: 8) {
                attendanceChip(
                    title: AttendanceStatus.present.title,
                    selected: selected == .present
                ) {
                    vm.toggle(.present, for: session)
                }

                attendanceChip(
                    title: AttendanceStatus.late.title,
                    selected: selected == .late
                ) {
                    vm.toggle(.late, for: session)
                }

                attendanceChip(
                    title: AttendanceStatus.absent.title,
                    selected: selected == .absent
                ) {
                    vm.toggle(.absent, for: session)
                }

                attendanceChip(
                    title: AttendanceStatus.excused.title,
                    selected: selected == .excused
                ) {
                    vm.toggle(.excused, for: session)
                }
            }
        }
        .padding(.vertical, 6)
    ...
    private func attendanceChip(
        ...
        Button(action: action) {
            Text(title)
                .font(.system(size: 13, weight: .medium))
                .foregroundColor(selected ? Color(.systemBackground) : .primary.opacity(0.7))
    ...
    @ViewBuilder
    private var perCourseCard: some View {
        VStack(alignment: .leading, spacing: 12) {
            Text("各課出席率")
                .font(.system(size: 14, weight: .medium))
                .foregroundColor(.primary.opacity(0.55))

            if vm.courseStats.isEmpty {
                Text("尚無出席資料")
                    .font(.system(size: 14))
                    .foregroundColor(.primary.opacity(0.55))
            } else {
                ForEach(vm.courseStats) { stat in
                    VStack(alignment: .leading, spacing: 5) {
                        Text(stat.courseName)
                            .font(.system(size: 14, weight: .medium))
                            .foregroundColor(.primary)

                        if let teacher = stat.teacher, !teacher.isEmpty {
                            Text(teacher)
                                .font(.system(size: 12, weight: .light))
                                .foregroundColor(.primary.opacity(0.5))
                        }

                        Text("\(percent(stat.rate))  (\(String(format: "%.1f", stat.presentEquivalent))/\(stat.totalForRate))")
                            .font(.system(size: 12, weight: .light))
                            .foregroundColor(.primary.opacity(0.7))
                    }
                    .frame(maxWidth: .infinity, alignment: .leading)
    ...
    private func moveSelectedDate(by days: Int) {
        guard let next = Calendar.current.date(byAdding: .day, value: days, to: vm.selectedDate) else { return }
        vm.selectedDate = next
    }

    private var formattedSelectedDate: String {
        Self.displayDateFormatter.string(from: vm.selectedDate)
    }

    private var selectedWeekday: String {
        Self.weekdayFormatter.string(from: vm.selectedDate)
    }
